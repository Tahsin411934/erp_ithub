<x-app-layout>
    <div class="mx-auto w-[98%]">
        <div class="flex justify-between items-center w-[90%] mx-auto my-8">
            <h1 class="text-2xl font-bold">Stationary Categories</h1>
        ...
        <div class="shadow-lg rounded-lg p-8 w-full overflow-x-auto">
            <table id="example" class="w-full text-left border-collapse">
                ...
                <tbody>
                    ...
                    <tr class="border-t">
                        <form action="{{ route('stationary-categories.update', $category->category_id) }}" method="POST" class="update-form">
                            @csrf
                            @method('PUT')
                            <td class="px-4 py-2">{{ $category->category_id }}</td>
                            <td class="px-4 py-2">
                                <textarea name="category_name" class="w-full border border-gray-300 rounded px-2 py-1 resize-none" disabled>{{ $category->category_name }}</textarea>
                            </td>
                        ...
                        <form action="{{ route('stationary-categories.destroy', $category->category_id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
    ...
    <!-- Add Category Modal -->
    <div id="category-modal" class="hidden fixed inset-0 z-50 overflow-y-auto flex items-center justify-center">
        <div class="relative w-full max-w-md mx-auto p-4">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form action="{{ route('stationary-categories.store') }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    <div>
                        ...
                        <input type="text" name="category_name" id="category_name" required
                            ...
                            placeholder="e.g. Pens, Notebooks, etc.">
                    </div>
